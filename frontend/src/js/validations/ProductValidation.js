const ProductValidation = (product) => {
  ...
  const idPattern = /^\d{5}$/; // It should be a string of 5 digits
  ...
  if (product.pricePerUnit == "") {
    errors.pricePerUnit = "מחיר זה שדה חובה";
  } else if (!/^\d+(\.\d{1,2})?$/.test(product.pricePerUnit)) {
    errors.pricePerUnit = "מחיר צריך להיות ספרות בלבד עם עד שני ספרות אחרי הנקודה";
  } else if (product.pricePerUnit <= 0) {
    errors.pricePerUnit = "מחיר לא יכול להיות שלילי או אפס";
  }
