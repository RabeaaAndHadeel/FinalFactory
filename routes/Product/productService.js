...
router.post("/createProduct", async (req, res) => {
  const { description, height, width, profileType, glassType, shutterType, pricePerUnit, quantity, idDoor, idWindow } = req.body;
  try {
    ...
    res.status(500).json({ error: 'Failed to save product to database', details: error.message }); // Send error response
  }
});
...
router.put("/updateProduct/:idProduct", (req, res) => {
  const { idProduct } = req.params; // Extract product ID from URL parameters
  const { description, glassType, height, pricePerUnit, profileType, quantity, shutterType, width, idDoor, idWindow } = req.body;
  const updatedProduct = { description, glassType, height, pricePerUnit, profileType, quantity, shutterType, width, idWindow, idDoor };
  ...
  db.query('UPDATE products SET ? WHERE idProduct = ?', [updatedProduct, idProduct], (error, results) => {
    if (error) {
...
//     res.status(500).json({ error: "Failed to calculate profiles", details: err.message }); // Send error response
//   }
// });
router.get("/calculate", async (req, res) => {
  try {
    const productIds = req.query.productIds 
      ? req.query.productIds.split(',').map(id => Number(id)) 
      : [];

    const profileDivisionFactor = 10; // Define your profileDivisionFactor here or get it from a configuration

    if (productIds.some(isNaN) || productIds.length === 0) {
      return res.status(400).json({ error: "Invalid or no product IDs provided" });
    }

    const data = await productData.calculateProfiles(productIds, profileDivisionFactor);

    res.json(data);
  } catch (err) {
    console.error("Failed to calculate profiles:", err); // Log error with stack trace
    res.status(500).json({ error: "Failed to calculate profiles", details: err.message });
  }
});
...
router.get("/cutting-plan", async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "Failed to get cutting plan", details: err.message }); // Send error response
  }
});
...
router.get("/associations", async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "Failed to fetch profile associations", details: err.message }); // Send error response
  }
});
