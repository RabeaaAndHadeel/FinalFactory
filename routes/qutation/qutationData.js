...
const createQuotation = (quotationData) => {
  ...
  return queryPromise(q, [idCustomer, customerName, date, totalPrice, discount,vat,notes])
    .then((result) => {
...
const updateQuotationById = (idQuotation, quotationData) => {
  const { idCustomer, customerName, date, totalPrice, discount, vat, notes } = quotationData;
...
const getQuotationsByCustomerId = async (idCustomer) => {
    ...
    const q = `SELECT * FROM quotations WHERE idCustomer = ?;`;
