...
// Function to delete a shutter by shutterId
const deleteShutterById = (shutterId) => {
  return new Promise((resolve, reject) => {
    const q = "DELETE FROM shutters WHERE shutterId = ?"; // Query to delete a shutter by ID
    db.query(q, [shutterId], (err, result) => {
      if (err) {
        console.error("Error deleting shutter:", err.message); // Log error message if query fails
        return reject(err); // Reject the promise with the error
      }
