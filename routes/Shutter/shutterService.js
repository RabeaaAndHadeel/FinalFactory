...
// Route to delete a shutter by ID
router.delete("/shutters/:shutterId", async (req, res) => {
  const { shutterId } = req.params; // Extract the shutter ID from request parameters
  try {
    await shutterController.deleteShutterById(shutterId); // Delete the shutter using the controller
    res.status(200).json({ message: "Shutter deleted successfully" }); // Send a success message with a 200 status code
  } catch (error) {
    res.status(500).json({ error: error.message }); // Handle any errors with a 500 Internal Server Error status and error message
  }
});
